...
class BasicIntegrationSuite  extends TempDirSuiteBase {
  ...
  test("Test Allocation & Finalization") {
    ...
    def allocObj(r: Radicle): Future[ObjectPointer] = {
      ...
      val ffp = sys0.lowLevelAllocateObject(r.systemTreeDefinitionPointer, ObjectRevision(0), BootstrapStoragePoolUUID,
  ...
  test("Test TestSystem Class") {
    ...
    def allocObj(r: Radicle): Future[ObjectPointer] = {
      ...
      val d = DataBuffer(ByteBuffer.allocate(5))
      val ffp = sys.lowLevelAllocateObject(r.systemTreeDefinitionPointer, ObjectRevision(0), BootstrapStoragePoolUUID,
                                    None, bootstrapPoolIDA, d)
      allocCount += 1
