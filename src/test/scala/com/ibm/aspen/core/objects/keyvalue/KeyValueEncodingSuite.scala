...
class KeyValueEncodingSuite extends FunSuite with Matchers {
  ...
  def encdec(kvos: KeyValueObjectState): KeyValueObjectState = {
    ...
    val storeStates = enc.zipWithIndex.map( t => KeyValueObjectStoreState(ida, t._2.asInstanceOf[Byte], t._1) ).toList
  ...
  test("Multiple Updates") {
    ...
    val storeStates = conjoined.zipWithIndex.map( t => KeyValueObjectStoreState(ida, t._2.asInstanceOf[Byte], t._1) ).toList
