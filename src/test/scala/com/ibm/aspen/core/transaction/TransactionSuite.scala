...
class TransactionSuite  extends AsyncFunSuite with Matchers {
  ...
  test("Nack prepare message") {
    ...
    val tx = new Transaction(crl, messenger, _ => (), _ => (), store, TransactionRecoveryState(
  ...
  test("Simple vote commit") {
    ...
    val tx = Transaction(crl, messenger, _ => (), _ => (), store, txd, LackContent)
  ...
  test("Vote commit with updates") {
    ...
    val tx = Transaction(crl, messenger, _ => (), _ => (), store, txd, mkLU(op.uuid))
  ...
  test("Vote abort when missing local updates") {
    ...
    val store = new NullDataStore(DataStoreID(poolUUID,0)) {
      ...
      override def lockTransaction(txd: TransactionDescription, updateData: List[LocalUpdate]): Future[List[ObjectTransactionError]] = {
  ...
  test("Vote abort on collision then vote commit after collision clears") {
    ...
    val store = new NullDataStore(DataStoreID(poolUUID,0)) {
      import NullDataStore._
      var ncalls = 0
      override def lockTransaction(txd: TransactionDescription, updateData: List[LocalUpdate]): Future[List[ObjectTransactionError]] = if (ncalls == 0) {
        ncalls += 1
        Future.successful(List(new TransactionCollision(op, collidingTxd, Some(NullDataStore.revision))))
      } else
  ...
  test("Receive Accept Response - commit state on transaction commit") {
    ...
    val store = new NullDataStore(DataStoreID(poolUUID,0)) {
      override def commitTransactionUpdates(txd: TransactionDescription, localUpdates: List[LocalUpdate]): Future[List[UUID]] = {
        stateCommitted = true
        Future.successful(Nil)
      }
    }
