...
import com.ibm.aspen.amoeba.{BaseFile, Directory, File, FileHandle, FileSystem, Symlink, FileType => AmoebaFileType}
import com.ibm.aspen.core.DataBuffer
import com.ibm.aspen.fuse.protocol.IOBufferManager
import com.ibm.aspen.fuse.protocol.messages._
import com.ibm.aspen.fuse.{DirEntry, FuseOptions, LinuxAPI, LowLevelFuseFilesystem, Response, Stat, FileType => FuseFileType}

import scala.concurrent.{ExecutionContext, Future}
import scala.util.{Failure, Success}

object FuseInterface {
  def toFuseFileType(ft: AmoebaFileType.Value): FuseFileType.Value = ft match {
    case AmoebaFileType.File            =>  FuseFileType.RegularFile
    case AmoebaFileType.Directory       =>  FuseFileType.Directory
    case AmoebaFileType.Symlink         =>  FuseFileType.SymbolicLink
    case AmoebaFileType.UnixSocket      =>  FuseFileType.UnixSocket
    case AmoebaFileType.CharacterDevice =>  FuseFileType.CharacterDevice
    case AmoebaFileType.BlockDevice     =>  FuseFileType.BlockDevice
    case AmoebaFileType.FIFO            =>  FuseFileType.Fifo
  }
            ...
            case dir: Directory => 
              dir.getEntry(request.name) onComplete {
                ...
                case Success(Some(pointer)) =>
                  if (pointer.ftype == AmoebaFileType.Directory)
