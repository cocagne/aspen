...
import com.ibm.aspen.amorfs.{BaseFile, Directory, File, FileHandle, FileSystem, Symlink, FileType => AmorfsFileType}
...
object FuseInterface {
  def toFuseFileType(ft: AmorfsFileType.Value): FuseFileType.Value = ft match {
    case AmorfsFileType.File            =>  FuseFileType.RegularFile
    case AmorfsFileType.Directory       =>  FuseFileType.Directory
    case AmorfsFileType.Symlink         =>  FuseFileType.SymbolicLink
    case AmorfsFileType.UnixSocket      =>  FuseFileType.UnixSocket
    case AmorfsFileType.CharacterDevice =>  FuseFileType.CharacterDevice
    case AmorfsFileType.BlockDevice     =>  FuseFileType.BlockDevice
    case AmorfsFileType.FIFO            =>  FuseFileType.Fifo
  }
            ...
            case dir: Directory => 
              dir.getEntry(request.name) onComplete {
                ...
                case Success(Some(pointer)) =>
                  if (pointer.ftype == AmorfsFileType.Directory)
