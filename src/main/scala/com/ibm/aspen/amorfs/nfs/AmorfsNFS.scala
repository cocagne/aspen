...
import com.ibm.aspen.amorfs
import com.ibm.aspen.amorfs.impl.SimpleFileHandle
import com.ibm.aspen.amorfs.{DirectoryEntry => _, Inode => _, _}
...
class AmorfsNFS(val fs: FileSystem,
  ...
  def list(inode: Inode, verifier: Array[Byte], cookie: Long): DirectoryStream = {
    ...
    val entries = dir.getContents().zipWithIndex.map { t =>
      val (amorfs.DirectoryEntry(name, iptr), cookie) = t
